...
void app_init(unsigned int wrappingEvents, string grammarFile) {
  ...
  if (!(c_code = fopen("Classifystart.c", "r"))) {
    cerr << "Could not read Classifystart.c\n";
    cerr << "Execution aborted.\n";
    exit(0);
  }
  ...
  if (!(c_code = fopen("Classifyend.c", "r"))) {
    cerr << "Could not read Classifyend.c\n";
    cerr << "Execution aborted.\n";
    exit(0);
  }
  while (fgets(buffer, 1000, c_code))
...
float objfunc(GAGenome &g) {
  ...
  if (phenotype->getValid()) {
    ...
    if (system("gcc -pipe individual.c gcc_GEClassify.o -lm\
				&& ./a.out > result") == -1) {
    ...
    //	if(mapper.getGenotype()->getWraps() > 0 )
    //	  fitness = 200;
