...
int main(int argc, char **argv) {
  ...
  while (!ga.done()) {
    ...
    cout.flush();
    // * Added by Sarmad
    GAListGenome<unsigned char> &genomee =
        (GAListGenome<unsigned char> &)ga.statistics().bestIndividual();
    // bestIndividual() returns reference to GAGenome instance which is
    // downcasted
