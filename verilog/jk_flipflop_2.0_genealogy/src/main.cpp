...
int CustomCrossover (const GAGenome &p1, const GAGenome &p2,  GAGenome *c1, GAGenome *c2)
{
 ...
 cout << "\n";
  ...
  //GAListGenome<unsigned char> &genome =
		//(GAListGenome<unsigned char> &) mom;
  //Assign genotype to mapper
  //mapper.setGenotype(genome);	 
  //int ef_size = mapper.getGenotype()->getEffectiveSize();  
  
  int ef_size = mom.getEffectiveSize(); 
  ...
  if (mom.getEffectiveSize() > mom.size())
  {
		...
		/* initialize random seed: */
  		srand (time(NULL));
  		int m_size = mom.size();
		int new_number = (rand() % (m_size + 1));
		
		cout << "Random: " << new_number << endl;
		cout << "Mom Size: " << mom.size() << endl;
		cout << "Point a: " << a << endl;
	}
	else
	{
		...
		a = GARandomInt (0, mom.getEffectiveSize());
		  ...
		  /* initialize random seed: */
  		srand (time(NULL));
  		int m_size = mom.getEffectiveSize();
		int new_number = (rand() % (m_size + 1));
		
		cout << "Random: " << new_number << endl;
		cout << "M_size: " << m_size << endl;
		cout << "ef_size: " << ef_size << endl;
		cout << "Mom Size: " << mom.size() << endl;
		cout << "Point a: " << a << endl;
	}
