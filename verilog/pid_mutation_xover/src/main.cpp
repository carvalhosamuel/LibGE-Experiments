...
int main(int argc, char **argv){
    ...
    bool runpid = false; //Use PID controller for mutation and xover?
    double m_rate = 0.0;
    double c_rate = 0.0;
    ...
    while(!ga.done()){
      ...
      // Initialize PID controller for sloth/PID
      PID pid = PID(1, 0.01, 0.0, 0.05, 0.001, 0.001);
      ...
      // PID controller for mutation and xover
      if(runpid==true){
        
        string str ="";
        
        // Reads the current diversity from temp file
        ifstream input_file("tmp/current_diversity");
        if (!input_file.is_open()) {
          cerr << "Could not open the diversity file" << endl;
          exit(EXIT_FAILURE);
        ...
        // At the first iteration we set the initial diversity, before using a fraction of it as setpoint
        if(j==0){
        ...
        // Defining a setpoint for the diversity
        setpoint = 0.8*initial;
        cout << "Setpoint: " << setpoint << endl;
        
        // Control action: the output from the PID controller
        double control_action = pid.calculate(setpoint, current_diversity);
        
        cout << "Control action: " << control_action << endl;
        
        // Sets new mutation rate
        cout << "Mutation rate GA before " << ga.pMutation() << endl;
        m_rate = ga.pMutation();
        m_rate += control_action;
        ga.pMutation(m_rate);
        cout << "Mutation rate GA after " << ga.pMutation() << endl;
        
        // Sets new crossover rate
        cout << "Crossover rate GA before " << ga.pCrossover() << endl;
      
        
        ga.pCrossover((ga.pCrossover() - control_action));
        cout << "Crossover rate GA after " << ga.pCrossover() << endl;
