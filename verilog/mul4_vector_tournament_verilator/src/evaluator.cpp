...
void evaluateTestbenchVerilator();
...
void createTestbench(GAPopulation &p){
  ...
  // Defining scale for verilator
  file << "`timescale 1ps/1ps" << endl;
  ...
  file << "\tif (vectornum == `TEST_COUNT) begin\n";
  file << "\t\t// Print all fitness scores to the console\n";
  file << "\t\tfor(int i = 0; i < `POPULATION_SIZE; i++) begin\n";
  file << "\t\t\t$display(\"%0d\",fitness[i]);\n";
  file << "\t\tend" << endl << endl;
  file << "\t\t$stop;" << endl << endl;
  file << "\tend" << endl << endl;
  file << "end" << endl << endl;
...
void evaluateTestbenchVerilator(){
  // Call the simulator - In this case Verilator
  system("verilator --sc -Wno-style -Wno-lint -Wno-context --exe sc_main.cpp tmp/testbench.sv");
  system("make -j -s -C obj_dir -f Vtestbench.mk Vtestbench");
  system("obj_dir/Vtestbench | tail +2 | head -n -2 > tmp/results");
...
void applySloth(float percentile){
  ...
  for (unsigned i=0; i<results.size(); i++){ // Go through the results
    ...
    if((results[i] <= sloth) && (index > 0)){ 
      ...
      cout<< "Swapping " << results[i] << " at position " << i << endl;
