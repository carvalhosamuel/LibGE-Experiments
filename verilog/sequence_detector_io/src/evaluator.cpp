...
void evaluateTestbenchIcarusDiff(GAPopulation &p);
...
void createIndividual(GAGenome &g, unsigned long id){
  ...
  if(phenotype->getValid())
  {
    ...
    string start_module = "module individual_0" + ports + "\n";
    string end_module = "\nendmodule";
...
void createTestbench(GAPopulation &p){
  ...
  file << "\treg [7:0] testvectors[0:(`TEST_COUNT-1)];" << endl;
...
void evaluateTestbenchIcarusDiff(GAPopulation &p){
  ...
  // For loop evaluates every individual in population
  for (int id = 0; id < p.size(); id++){
  
  // System call to Icarus 
  string call = "iverilog '-g2012' tmp/individual_"+ to_string(id) +".sv original_individual/new_tb.sv -o a.out && vvp a.out > tmp/out_"+ to_string(id) +"";
  system(call.c_str());
  
  // System call for the diff commmand: compares the output of current individual to the test vectors and saves into results file
  string diff_call = "diff tmp/out_"+ to_string(id) + " original_individual/test_vectors.tv | grep \"^>\" | wc -l >> tmp/results";
  system(diff_call.c_str());
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 1.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 2.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 3.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 4.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 5.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
  ...
  if(port_file.is_open())
  {
    while(getline(port_file,line))
    {
      if(line.find("input") != string::npos || line.find("output") != string::npos){
        ...
        cerr << "Error parsing port module connections 6.\n";
        cerr << "Execution aborted.\n";
        exit(1);
      }
    }
