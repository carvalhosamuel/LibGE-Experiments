...
int AssignFitness() {
  ...
  for (i = 0; i < FitnessCases; i++)
    if ((((Evolved[i] > 0) && (Class[i] == 1))) ||
        ((Evolved[i] <= 0) && (Class[i] == 0)))
      hits++;
  return hits;
}
...
double myadd(double a1, double a2) { return a1 + a2; }

double mysub(double a1, double a2) { return a1 - a2; }

double mymul(double a1, double a2) { return (a1 * a2); }
